...
##constants## got from the log file

A = 0.529177249 # bohr to angstrom 1 bohr = 0.529177249 angstrom
O = 15.99491**(1/2) # mass**(1/2) of oxygen
H = 1.00782**(1/2) # mass**(1/2) of hydrogen
E = 27.2114079527 # hartree to eV 1 hartree = 27.2114079527 eV
...
#extract internal coordinates from zmat file
...
with open( './data/raw.c1.int.log' , 'r') as f:
    ...
    #R0_amu_vec is in mass**(1/2) * bohr unit
    ...
    #force_3Nx3N is non-mass weighted hessian matrix (bohr unit)
    force_3Nx3N = np.mat([
    ...
    #get Q_i from the raw file and covert to mass*(1/2)*bohr unit -> normal mode vectors
    ...
    #Q_vec is mass**(1/2)*bohr unit
    Q_vec = np.array([modify_list(Q_bohr[i], O, H) for i in range(3)])

###########################################################
###########################Method##########################

#get the energy from the xyz file -> direct method
def direct(xyz_file):
...
#internal morse method (same as internal method but with morse potential shape, this can be done with three data points)
...
#write the file with the new energy values
...
#erase the force information from the xyz file (optional)
...
##force eraser usage
# erase_force_info(xyz_file, './data/test.xyz')
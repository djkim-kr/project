...
random_number_1 = np.random.uniform(-0.4, 0.4, 3000)
random_number_2 = np.random.uniform(-0.2, 0.3, 3000)
random_number_3 = np.random.uniform(-0.3, 0.3, 3000)
temp =[]

for i in range(len(random_number_1)):
    temp_1 = (init_coordinate_bohr 
                + (random_number_1[i] * Q_1_bohr_vec)
                + (random_number_2[i] * Q_2_bohr_vec)
                + (random_number_3[i] * Q_3_bohr_vec))
    temp_1_ang = temp_1 * A
...
with open(file_log, 'w') as f:
    for i in range(len(temp)):
        f.write('3\n')
        f.write(f'Lattice="15.0 0.0 0.0 0.0 15.0 0.0 0.0 0.0 15.0" Properties=species:S:1:pos:R:3 energy={np.round(random_number_1[i],3)}/ {np.round(random_number_2[i],3)}/ {np.round(random_number_3[i],3)} pbc="T T T"\n')

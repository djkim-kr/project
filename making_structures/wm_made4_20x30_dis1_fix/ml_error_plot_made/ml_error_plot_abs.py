...
def generate_contour_plot(word, ax, vmin, vmax):
    ...
    contour2 = ax.contourf(xi, yi, Z, levels=256, cmap='Blues'
                           , vmin=vmin, vmax=vmax)
    
    ax.scatter(x=made_dis_2, y=made_ang, s=5, color='yellow', label='original', alpha=0.5)

    ax.axhline(y=eq_ang, color='red', linestyle='--', alpha=0.3)
    ...
    ax.scatter(dis, ang, color='k',alpha=0.5, s=3)
    # ax.set_xlim(eq_dis - 0.2, eq_dis + 0.2)
    # ax.set_ylim(eq_ang - 20, eq_ang + 20)
    ax.set_xlim(0.7,1.4)
    ax.set_ylim(60,150)
    ax.set_title(f'60_abs_made_{word}_ml_pot_contour')
...
colormapping = cm.ScalarMappable(norm=norm, cmap='Blues')
fig.colorbar( colormapping, ax=axs, fraction=0.05, pad=0.1, label='E(meV)')

plt.savefig('./figure/60_made_abs_ml_error_1000.png',dpi=300)
# plt.show()
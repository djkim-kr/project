...
def generate_contour_plot(word, ax, vmin, vmax):
    ...
    xi, yi = np.linspace(dis_1.min(), dis_2.max(), 100), np.linspace(dis_2.min(), dis_2.max(), 100)
    ...
    ax.scatter(x=made_dis_1, y=made_dis_2, s=5, color='white', label='original', alpha=0.5)

    ax.axhline(y=eq_dis, color='red', linestyle='--', alpha=0.5)
    ...
    ax.scatter(dis_1, dis_2, color='k', alpha=0.5, s=5)
    # eq_dis = 0.95883
    # eq_ang = 104.34240
    ax.set_xlim(0.7,1.4)
    ax.set_ylim(0.7,1.4)  
    ...
    ax.set_title(f'0104_original_{word}_ml_pot_contour')
...
plt.savefig('./figure/0104_ml_potential_scatter_contour.png',dpi=300)
plt.show()